...
func Main(archInit func(*Arch)) {
	timings.Start("fe", "init") // 编译器前端初始化
	...
	timings.Start("fe", "loadsys") // 加载运行时系统函数
	loadsys()

	timings.Start("fe", "parse")   // 词法分析/语法分析
	// 解析文件名
	...
	timings.Start("fe", "typecheck", "top1") // 开始类型检查 - top1：常量，类型，函数名和类型
	...
	// Don't use range--typecheck can add closures to xtop.
	timings.Start("fe", "typecheck", "top2") // 类型检查 - top2:类型赋值
	for i := 0; i < len(xtop); i++ {
		n := xtop[i]
		if op := n.Op; op == ODCL || op == OAS || op == OAS2 || op == ODCLTYPE && n.Left.Name.Param.Alias {
			xtop[i] = typecheck(n, ctxStmt)
	...
	// Don't use range--typecheck can add closures to xtop.
	timings.Start("fe", "typecheck", "func") // 类型检查 - top3: 函数体
	var fcount int64
	for i := 0; i < len(xtop); i++ {
		n := xtop[i]
	...
	timings.Start("fe", "capturevars") // 捕获已经关闭的变量
	for _, n := range xtop {
	...
	// Phase 5: Inlining
	timings.Start("fe", "inlining") // 内联类型
	...
	timings.Start("fe", "xclosures") // 转换闭包的主体为何时的引用捕获变量
	for _, n := range xtop {
		if n.Op == ODCLFUNC && n.Func.Closure != nil {
			Curfn = n
			transformclosure(n)
	...
	timings.Start("be", "compilefuncs") // 后端: 编译顶层函数
	fcount = 0
	for i := 0; i < len(xtop); i++ {
		n := xtop[i]
		// 如果是函数; 函数编译
	...
	compileFunctions() // compileSSA -> buildssa
	...
	timings.Start("be", "externaldcls") // 检查外部声明
	...
	timings.Start("be", "dumpobj") // 写对象数据到硬盘
	dumpdata()

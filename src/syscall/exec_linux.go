...
// of this and forkAndExecInChild separate. /// 栈帧分离
//
//go:noinline
//go:norace
func forkAndExecInChild1(argv0 *byte, argv, envv []*byte, chroot, dir *byte, attr *ProcAttr, sys *SysProcAttr, pipe int) (r1 uintptr, err1 Errno, p [2]int, locked bool) {
	...
	// synchronizing writing of User ID/Group ID mappings. /// forkExecPipe 管道
	if sys.UidMappings != nil || sys.GidMappings != nil {
	...
	default:
		r1, _, err1 = RawSyscall6(SYS_CLONE, uintptr(SIGCHLD)|sys.Cloneflags, 0, 0, 0, 0, 0) // 克隆子进程
	}
	if err1 != 0 || r1 != 0 { // 父进程立即返回
	...
	// Fork succeeded, now in child. /// **************** 这之后都是子进程 *****************
	...
	// Pass 1: look for fd[i] < i and move those up above len(fd) /// 找到 fd[i] < i 的文件描述符；并将其提升到最小的一个文件描述符
	...
	for i = 0; i < len(fd); i++ {
		...
		if fd[i] == int(i) { // 必须完全一致
			// dup2(i, i) won't clear close-on-exec flag on Linux,
			// probably not elsewhere either.
			_, _, err1 = RawSyscall(fcntl64Syscall, uintptr(fd[i]), F_SETFD, 0) // 如果设置为fcntl(fd, F_SETFD, 0);那么本fd将保持打开状态复制到exec创建的新进程中
			if err1 != 0 {
				goto childerror
			}
			continue

...
type Msghdr struct {
	Name       *byte      /// 协议地址
	Namelen    uint32     /// 协议地址长度
	Pad_cgo_0  [4]byte    ///
	Iov        *Iovec     /// scatter/gather array 输入输出缓冲区数组
	Iovlen     uint64
	Control    *byte      /// ancillary data
	Controllen uint64     /// length of ancillay data
	Flags      int32      /// flags returned by recvmsg [MSG_BCAST/MSG_MCAST/MSG_TRUNC/MSG_CTRUNC/MSG_EOR/MSG_OOB/MSG_NOTIFICATION]
	Pad_cgo_1  [4]byte
}

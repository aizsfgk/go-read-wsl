...
type ClientTrace struct {
	...
	GetConn func(hostPort string) /// 连接创建前

	// GotConn is called after a successful connection is
	// obtained. There is no hook for failure to obtain a
	// connection; instead, use the error from
	// Transport.RoundTrip.
	GotConn func(GotConnInfo)    /// 连接成功创建后
	...
	PutIdleConn func(err error)   /// 连接返回给空闲池

	// GotFirstResponseByte is called when the first byte of the response
	// headers is available.
	GotFirstResponseByte func()   /// 当第一个响应字节可用

	// Got100Continue is called if the server replies with a "100
	// Continue" response.
	Got100Continue func()         /// 100个联系响应
	...
	DNSStart func(DNSStartInfo)   /// DNS解析开始

	// DNSDone is called when a DNS lookup ends.
	DNSDone func(DNSDoneInfo)     /// DNS解析结束
	...
	// all request headers. /// Transport 已经写入全部的请求头
	WroteHeaders func()
	...
	WroteRequest func(WroteRequestInfo)  /// 16个Hook函数
}

...
var trailingPort = regexp.MustCompile(`:([0-9]+)$`)   // :以数字结尾

var osDefaultInheritEnv = func() []string {          // 默认动态库路径
	switch runtime.GOOS {
	case "darwin":
...
func (h *Handler) ServeHTTP(rw http.ResponseWriter, req *http.Request) { // 处理serverHTTP
	root := h.Root
	if root == "" {
		root = "/"
	}
	...
	_, err = io.Copy(rw, linebody)  /// 响应客户端
	if err != nil {

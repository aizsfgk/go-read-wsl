...
const (
	...
	///
	///  不仅仅是G的状态；它还看起来像一把关于g stack 的锁；因此它能够执行用户代码
	///
	...
	// _Gwaiting means this goroutine is blocked in the runtime. /// 阻塞在运行时
	// It is not executing user code. It is not on a run queue,
	// but should be recorded somewhere
	/// 它被记录在一些地方：例如 一个channel的等待队列
	//(e.g., a channel wait
...
const (
	...
	/// _Prunning 被一个M拥有，并且被用来执行用户代码，或者调度器。
	_Prunning
	...
	///
	/// _Psyscall 意味着 一个P没有运行用户代码。它具有亲和力对一个陷入系统调用的M,但是
	/// M并没有拥有P, P可以被其他M窃取。这是相似的如同_Pidle,但其使用轻量的转换和管理
	/// M的亲和力。
	/// P脱离_Psyscall状态必须使用CAS命令，这意味着P被窃取了，或者被先前的M重新持有。
	/// 注意：这里有个ABA的风险：好像一个M成功的执行了ABA状态转换，它原始的P变为了_Prunning
	/// 在一个系统调用之后，必须理解的是这个P可以被其他M临时使用一会儿。
	_Psyscall
	...
	///
	/// _Pgcstop 意味着一个P被中断为了STW, 被M（已经停止了世界的)拥有。
	/// 停止了世界的M, 继续使用它的P,甚至在_Pgcstop下。从_Prunning转换到_Pgcstop
	/// 造成了一个M释放了它的P并且Park(暂停)
	_Pgcstop
	...
	// (e.g., trace buffers).
	///
	/// _Pdead 意味着 P不再使用，GOMAXPROCS收缩；只有GOMAXPROCS增长后，才再次使用。
	/// 一个deadP被剥夺了大部分资源，只有一小点东西保留，例如：trace buffers
	_Pdead
)
...
type gobuf struct { /// gobuf结构体用于保存goroutine的调度信息
...
///
/// sudog是必须的，因为g<->管道相关同步对象是非常多的。一个g可以在许多等待列表中，
/// 因此有许多sodugs 为一个g; 同时，去多gs能够等待在相同的同步对象上，因此这有
/// 许多sudogs为一个对象
///
// sudogs are allocated from a special pool. Use acquireSudog and
// releaseSudog to allocate and free them.
///
/// sudogs 被分配从一个特殊的池上，使用acquireSudog 和 releaseSudog 去
/// 分配和释放它们。
///
type sudog struct {
	...
	next *sudog
	prev *sudog
	elem unsafe.Pointer // data element (may point to stack) /// 数据元素可以指向一个栈
	...
	isSelect bool   /// 标明g是否是select语句的一部分
...
type stack struct {
	lo uintptr /// 栈顶，执行内存低地址
	hi uintptr /// 栈低，指向内存高地址
}
...
type g struct {
	...
	goid         int64
	schedlink    guintptr   /// 代表一个goroutine指针
	...
	// areas of the stack.
	/// true: 栈复制的时候需要获取c hannel-lock 来保护栈区域
...
type m struct {
	g0      *g     // goroutine with scheduling stack                                               /// 调度栈
	morebuf gobuf  // gobuf arg to morestack                                                        /// 将g->sched参数复制到morebuf
	divmod  uint32 // div/mod denominator for arm - known to liblink                                ///

	// Fields not known to debuggers.
	procid        uint64       // for debuggers, but offset not hard-coded                          /// 进程/线程ID
	gsignal       *g           // signal-handling g                                                 /// 信号处理g
	goSigStack    gsignalStack // Go-allocated signal handling stack                                /// 信号处理栈
	sigmask       sigset       // storage for saved signal mask                                     /// signal mask

	/// 通过TLS实现m结构体对象与工作线程之间的绑定
	tls           [6]uintptr   // thread-local storage (for x86 extern register) /// 6个指针
	mstartfn      func()                                                                            /// m启动函数
...
type p struct {
	...
	link        puintptr      ///
	schedtick   uint32     // incremented on every scheduler call         /// 每次call调度器 +1
	syscalltick uint32     // incremented on every system call            /// 每次syscall +1
	sysmontick  sysmontick // last tick observed by sysmon                /// 最后的 sysmon tick
	...
	// Queue of runnable goroutines. Accessed without lock.     /// p 本地运行队列
	runqhead uint32
	runqtail uint32
	...
	// traceSweep indicates the sweep events should be traced. /// 清扫事件应该被跟踪 /// 垃圾回收，是伴随每个P的
	...
	// worker started.
	gcMarkWorkerStartTime int64 /// GC 标记工作开始时间

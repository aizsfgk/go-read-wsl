...
type hchan struct {
	...
	recvq    waitq  // list of recv waiters  /// 接收等待goroutine
	sendq    waitq  // list of send waiters  /// 发送等待goroutine

	// lock protects all fields in hchan, as well as several
	// fields in sudogs blocked on this channel.
	//
	// Do not change another G's status while holding this lock
	// (in particular, do not ready a G), as this can deadlock /// 死锁的发生
...
func chansend(c *hchan, ep unsafe.Pointer, block bool, callerpc uintptr) bool {
	...
	if sg := c.recvq.dequeue(); sg != nil {
		/// 发现一个等待的receiver, 我们直接将值传递给它，绕过channel buffer
	...
	if c.qcount < c.dataqsiz {
		...
		qp := chanbuf(c, c.sendx) /// 有剩余空间，存入c.buf
		if raceenabled {
			raceacquire(qp)
			racerelease(qp)
		}
		typedmemmove(c.elemtype, qp, ep) /// 将要发送的数据拷贝到buf中
		c.sendx++
	...
	gp.waiting = mysg
	gp.param = nil
	c.sendq.enqueue(mysg) /// 加入到c的sendq中
	/// 当前go park
...
func sendDirect(t *_type, sg *sudog, src unsafe.Pointer) {
	...
	typeBitsBulkBarrier(t, uintptr(dst), uintptr(src), t.size) /// 内存屏障
	// No need for cgo write barrier checks because dst is always
	// Go memory.
	memmove(dst, src, t.size) /// 内存拷贝；直接写入接收方的执行栈！
}
...
func recv(c *hchan, sg *sudog, ep unsafe.Pointer, unlockf func(), skip int) {
	...
	/// 唤醒这个g
	/// 无缓冲 Channel 的接收方会先从发送方栈拷贝数据后，发送方才会被放回调度队列中，等待重新调度
	goready(gp, skip+1)
}

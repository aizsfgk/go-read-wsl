...
/// 1. 提交一个 goroutine 到一个本地队列
/// 2. 开启 #StoreLoad-style memory barrier
/// 3. 检查 sched.nmspinning
/// 4. M 从 spinning 转换为 non-spinning
//  5. 减少 sched.nmspingning
//  6. #StoreLoad-style memory barrier
/// 7. 检查全部的 per-P 工作队列，为了找到一个工作
/// 8. *** 注意这些不适用全局队列 ***
...
///
/// 将当前goroutine设为一个等待状态；然后调用unlockf
/// 如果unlockf返回false, 恢复这个goroutine
/// unlockf 不能访问G的栈，它可以被修改移动在gopark和unlockf之间
/// 原因解释了，为何goroutine被park
///
func gopark(unlockf func(*g, unsafe.Pointer) bool, lock unsafe.Pointer, reason waitReason, traceEv byte, traceskip int) {
	...
	mcall(park_m) /// parkm ; mcall 转换到g0栈
}
...
func allgadd(gp *g) {
	...
	lock(&allglock)
	allgs = append(allgs, gp)               /// 增加allgadd
	allglen = uintptr(len(allgs))           /// 设定长度
	unlock(&allglock)
}
...
func schedinit() {
	...
	sched.maxmcount = 10000   /// m最多10000个
...
func stopTheWorld(reason string) {
	...
	systemstack(func() {
		...
		casgstatus(gp, _Grunning, _Gwaiting) /// stopTheWorld
		stopTheWorldWithSema()
		casgstatus(gp, _Gwaiting, _Grunning) /// stopTheWorld
	})
}
...
func stopTheWorldWithSema() {
	...
	for _, p := range allp {
		...
		if s == _Psyscall && atomic.Cas(&p.status, s, _Pgcstop) { /// /// stopTheWorldWithSema
			if trace.enabled {
				traceGoSysBlock(p)
				traceProcStop(p)
			}
	...
	for {
		...
		p.status = _Pgcstop /// stopTheWorldWithSema
		sched.stopwait--
	}
	...
	if sched.stopwait != 0 {
		...
		for _, p := range allp {
			if p.status != _Pgcstop { /// stopTheWorldWithSema
...
func mstart() {   /// 启动m0
	...
	//println("proc.go=>mstart=>_g_.m", _g_.m)
	//println("proc.go=>mstart=>_g_.m.g0", _g_.m.g0)
	//println("proc.go=>mstart=>_g_.m.g0.m", _g_.m.g0.m)

	osStack := _g_.stack.lo == 0 /// 是否是系统栈
	if osStack {
...
func mstart1() {
	_g_ := getg()
	//println("proc.go=>mstart1=>_g_: ", _g_)
	//println("proc.go=>mstat1=>_g_.m.g0.m: ", _g_.m.g0.m)
	if _g_ != _g_.m.g0 {
	...
	/// getcallerpc : mstart函数内下一条指令，也就是mexit(osStack)
	/// getcallersp : mstart的栈顶
...
func needm(x byte) {
	...
	casgstatus(mp.curg, _Gdead, _Gsyscall) /// needm
...
func oneNewExtraM() {
	...
	casgstatus(gp, _Gidle, _Gdead) /// oneNewExtraM
	gp.m = mp
	mp.curg = gp
	mp.lockedInt++
	mp.lockedg.set(gp)
	...
	// has the same effect.
	atomic.Xadd(&sched.ngsys, +1) /// oneNewExtraM
...
func dropm() {
	...
	casgstatus(mp.curg, _Gsyscall, _Gdead) /// dropm
	mp.curg.preemptStop = false
	atomic.Xadd(&sched.ngsys, +1) /// dropm
...
func newm(fn func(), _p_ *p, id int64) {
	...
	if gp := getg(); gp != nil && gp.m != nil && (gp.m.lockedExt != 0 || gp.m.incgo) && GOOS != "plan9" {
		...
		mp.schedlink = newmHandoff.newm   /// 拉链
...
func stopm() {
	...
	lock(&sched.lock)
	mput(_g_.m)                  /// stopm的核心是调用mput把m结构体对象放入sched的midle空闲队列，然后通过notesleep(&m.park)函数让自己进入睡眠状态。
	unlock(&sched.lock)          /// note是go runtime实现的一次性睡眠和唤醒机制，一个线程可以通过调用notesleep(*note)进入睡眠状态，而另外一个线程则可以通过notewakeup(*note)
	notesleep(&_g_.m.park)
	noteclear(&_g_.m.park)
	acquirep(_g_.m.nextp.ptr())  /// 返回后，需要再次绑定一个P
	_g_.m.nextp = 0
}
...
func execute(gp *g, inheritTime bool) {
	_g_ := getg() /// g0
	//println("execute=> g0: ", &g0)
	//println("execute=> m0: ", &m0)
	//println("_g_ e: ", _g_)           // 执行g
	//println("_g_.m e: ", _g_.m)       // 执行

	// Assign gp.m before entering _Grunning so running Gs have an
	// M.

	/// 初始化时: 将 g1 和 m0 关联起来
	_g_.m.curg = gp         /// core 很重要啊??? [gp是从P队列里获取到的g]

	gp.m = _g_.m ///  把g和m关联起来
	casgstatus(gp, _Grunnable, _Grunning) /// execute
	gp.waitsince = 0
	gp.preempt = false
...
///
/// 添加每一个可运行的在list中的G到一些运行队列，并清空gList
/// 如果没有当前P, 则将G放入全局队列。npile个M将启动运行G.
///
///
///
func injectglist(glist *gList) { /// 注入
	if glist.empty() {
		return
	}
	if trace.enabled {
...
/// 之后工作线程一直循环执行着3～5这一调度循环直到进程退出为止
...
///
func schedule() {
	_g_ := getg() /// 每个工作线程m对应的g0; 初始化时m0的g0
...
top:
	...
	if trace.enabled || trace.shutdown {
		gp = traceReader()
		if gp != nil {
			casgstatus(gp, _Gwaiting, _Grunnable) /// schedule =》 trace.enabled
			traceGoUnpark(gp, 0)
			tryWakeP = true
		}
	}
	...
	execute(gp, inheritTime) /// schedule ---> gogo代码
}

// dropg removes the association between m and the current goroutine m->curg (gp for short).
// Typically a caller sets gp's status away from Grunning and then
// immediately calls dropg to finish the job. The caller is also responsible /// 响应的
...
func park_m(gp *g) {
	...
	casgstatus(gp, _Grunning, _Gwaiting) /// 原始g设置为等待

	dropg() /// 删除m和当前g的相关性
	///
	/// 网络异步IO; 会将阻塞的G； 转交给netpoller代理；当相关的sysfd可读写的时候； injectList G; 开始运行这个G
	///
	///
	///
	...
	if fn := _g_.m.waitunlockf; fn != nil {
		...
		if !ok {
			...
			casgstatus(gp, _Gwaiting, _Grunnable) /// park_m
...
func goyield_m(gp *g) {
	...
	pp := gp.m.p.ptr()
	casgstatus(gp, _Grunning, _Grunnable) /// goyield_m
	dropg()
	runqput(pp, gp, false)
	schedule()
}
...
func goexit0(gp *g) { /// gp 是用户线程
	...
	gfput(_g_.m.p.ptr(), gp) /// g 放入freeList
...
func save(pc, sp uintptr) {
	...
	_g_.sched.pc = pc  /// 再次运行的执行地址
	_g_.sched.sp = sp  /// 再次运行的栈顶
	_g_.sched.lr = 0
	_g_.sched.ret = 0
...
func reentersyscall(pc, sp uintptr) {
	...
	_g_.syscallsp = sp
	_g_.syscallpc = pc
	casgstatus(_g_, _Grunning, _Gsyscall) /// reentersyscall
...
func entersyscallblock() {
	...
	casgstatus(_g_, _Grunning, _Gsyscall) /// entersyscallblock
	if _g_.syscallsp < _g_.stack.lo || _g_.stack.hi < _g_.syscallsp {
		systemstack(func() {
...
func exitsyscall() {
	...
	if exitsyscallfast(oldp) {
		...
		casgstatus(_g_, _Gsyscall, _Grunning) /// exitsyscall
...
func exitsyscall0(gp *g) {
	...
	casgstatus(gp, _Gsyscall, _Grunnable) /// exitsyscall0
	dropg()
	lock(&sched.lock)
	var _p_ *p
	if schedEnabled(_g_) {
...
func beforefork() {  /// 保存信号
	...
	gp.m.locks++
	msigsave(gp.m) /// 新进程信号置空
	sigblock()     /// 保存信号

	// This function is called before fork in syscall package.
	// Code between fork and exec must not allocate memory nor even try to grow stack. /// 不能分配内存，也不能增加堆栈
...
// Called from syscall package after fork in child. 子进程中调用
...
///
//go:nosplit
func newproc(siz int32, fn *funcval) { /// siz 参数字节大小； fn
	...
	systemstack(func() {
		newg := newproc1(fn, argp, siz, gp, pc)  /// 新的goroutine；g1 # 1. 新创建g
...
func newproc1(fn *funcval, argp unsafe.Pointer, narg int32, callergp *g, callerpc uintptr) *g {
	...
	sp := newg.stack.hi - totalSize /// sp增加一个返回地址的空间；
	...
	/// 调整sched成员和newg的栈
	gostartcallfn(&newg.sched, fn) /// fn : funcval{runtime.main}
	...
	if isSystemGoroutine(newg, false) {
		atomic.Xadd(&sched.ngsys, +1) /// newproc1
	}
...
// 初始化initialize的pp, 可能时一个新建的p或者一个之前已经销毁的p; 转换其状态为_Pgcstop
...
func (pp *p) destroy() {
	...
	if gp := pp.gcBgMarkWorker.ptr(); gp != nil {
		casgstatus(gp, _Gwaiting, _Grunnable) /// destroy
		if trace.enabled {
			traceGoUnpark(gp, 0)
		}
		globrunqput(gp)
...
func procresize(nprocs int32) *p {
	...
	for i := nprocs - 1; i >= 0; i-- {
		p := allp[i]
		if _g_.m.p.ptr() == p { /// 跳过p0; //allp[0]跟m0关联了，所以是不能放任
			continue
		}
		p.status = _Pidle
		if runqempty(p) { /// 如果本地运行队列为空
			pidleput(p) /// 将p放入全局空闲列表
		} else {
			p.m.set(mget())
	...
	//println("proc.go=>procresize=>runnablePs: ",runnablePs)
	return runnablePs
}
	...
	for {
		...
		if netpollinited() && lastpoll != 0 && lastpoll+10*1000*1000 < now {
			...
			if !list.empty() {
				...
				incidlelocked(-1)
				injectglist(&list) /// 适当的时候；将就绪的G放入队列
				incidlelocked(1)
			}
		}
		if next < now {

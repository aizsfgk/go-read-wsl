...
// On systems with link registers, the prologue for a non-leaf function stores the  /// [prologue: 序言]
...
func traceback(pc, sp, lr uintptr, gp *g) {
	traceback1(pc, sp, lr, gp, 0) /// gorounte 记录
}
...
func goroutineheader(gp *g) {
	...
	gpstatus &^= _Gscan // drop the scan bit /// 剔除扫描位

	// Basic string status
	var status string
	if 0 <= gpstatus && gpstatus < uint32(len(gStatusStrings)) { /// 状态合法

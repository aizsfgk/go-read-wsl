...
// Goroutine preemption : G 抢占
//
// A goroutine can be preempted at any safe-point. Currently, there
// are a few categories of safe-points:
/// 一个goroutine 能够被抢占在任何的安全点。当前有一些安全点：
//
// 1. A blocked safe-point occurs for the duration that a goroutine is
//    descheduled, blocked on synchronization, or in a system call. /// 阻塞在一个系统调用上
//
// 2. Synchronous safe-points occur when a running goroutine checks
//    for a preemption request.  /// 同步安全点发生，当一个goroutine检测一个抢占请求
//
...
///
/// suspendG 暂停G gp 在一个安全点，并返回这个已经暂停的G的状态。
/// 这个调用者能够读取访问这个G直到G调用了resumeG
///
///
//
...
func canPreemptM(mp *m) bool {
	return mp.locks == 0 && mp.mallocing == 0 && mp.preemptoff == "" && mp.p.ptr().status == _Prunning /// canPreemptM
}
...
func asyncPreempt2() {
	...
	if gp.preemptStop { /// 合法停止
		mcall(preemptPark)
	} else {
		mcall(gopreempt_m) /// Accept/read 抢占m
	}
...
func init() {
	f := findfunc(funcPC(asyncPreempt)) /// 先获取函数的PC; 然后获取函数信息; 然后获取函数的最大栈大小

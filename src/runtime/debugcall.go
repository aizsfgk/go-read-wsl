...
func debugCallWrap(dispatch uintptr) {
	...
	mcall(func(gp *g) {
		...
		casgstatus(gp, _Grunning, _Gwaiting) /// debugCallWrap
		dropg()
...
func debugCallWrap1(dispatch uintptr, callingG *g) {
	...
	mcall(func(gp *g) {
		...
		casgstatus(gp, _Grunning, _Grunnable) /// debugCallWrap1
		dropg()
		lock(&sched.lock)
		globrunqput(gp)
		unlock(&sched.lock)
		...
		casgstatus(callingG, _Gwaiting, _Grunnable) /// debugCallWrap1
		execute(callingG, true) /// debugCallWrap1
	})
}

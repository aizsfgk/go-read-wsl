...
TEXT runtime·rt0_go(SB),NOSPLIT,$0
	...
	MOVQ	$runtime·g0(SB), DI            /// 将g0地址放入DI
	LEAQ	(-64*1024+104)(SP), BX         /// 大约64KB
	MOVQ	BX, g_stackguard0(DI)          /// g.stackguard0指向栈顶
	MOVQ	BX, g_stackguard1(DI)          /// g.stackguard1指向栈顶
	MOVQ	BX, (g_stack+stack_lo)(DI)     /// g0.stack.stack_lo 保存g0栈低地址； 栈顶
	MOVQ	SP, (g_stack+stack_hi)(DI)     /// g0.stack.stack_hi 保存g0栈高地址； 栈底

	// find out information about the processor we're on
	MOVL	$0, AX
	CPUID              /// CPUID隐式使用EAX寄存器来确定返回信息的主类别
	MOVL	AX, SI
	CMPL	AX, $0
	JE	nocpuinfo
...
notintel:
	...
	MOVL	$1, AX
	CPUID
	MOVL	AX, runtime·processorVersionInfo(SB)  /// 处理器版本信息:全局变量：processorVersionInfo
...
needtls:
	...
	LEAQ	runtime·m0+m_tls(SB), DI     /// m0.tls 放入 DI
	CALL	runtime·settls(SB)           /// 设置fs寄存器基地址为 m0.tls[1]
...
ok:
    ...
    /// ************** 03. 创建m0栈 ************* ///
	MOVL	16(SP), AX		// copy argc   /// 赋值argc,argv到m0栈
	MOVL	AX, 0(SP)
	...
	/// ************* 04. 参数初始化 ************* ///
	CALL	runtime·args(SB)           /// 参数初始化; 空函数???

	/// ************* 05. 系统初始化 *********** ///
	CALL	runtime·osinit(SB)         /// os初始化: cpu个数；物理页大小；架构初始化

    /// ************* 06. 调度器初始化[core] ************* ///
	CALL	runtime·schedinit(SB)      /// 调度器初始化; 获取m的g0； 创建pList; 绑定GMP关系
    ...
    ///
	// start this M
	CALL	runtime·mstart(SB)        /// m0 启动；并调度执行g1
...
DATA	runtime·mainPC+0(SB)/8,$runtime·main(SB) /// main入口的PC地址
...
TEXT runtime·gogo(SB), NOSPLIT, $16-8
	...
	MOVQ	gobuf_pc(BX), BX    /// 跳到pc执行 /// 初始化这里存的funcval{runtime.main}
	JMP	BX                      /// 去执行
...
// to keep running g.
/// 转换到g0栈，调用fn(g1)
/// 它永远不返回，它应该调用gogo(&g->sched) 去运行一个newg
///
/// mcall做的事情跟gogo函数完全相反
///
TEXT runtime·mcall(SB), NOSPLIT, $0-8
	...
	JMP	AX
	MOVQ	SI, g(CX)	// g = m->g0    /// 恢复g0
	/// 恢复g0的栈顶指针到CPU的rsp积存，这一条指令完成了栈的切换，从g的栈切换到了g0的栈
	MOVQ	(g_sched+gobuf_sp)(SI), SP	// sp = m->g0->sched.sp /// 转换到g0
	PUSHQ	AX /// fn的参数g入栈 /// goexit0 gp 参数，用户态goroutine指针
...
TEXT runtime·systemstack(SB), NOSPLIT, $0-8
	...
	MOVQ	$runtime·systemstack_switch(SB), SI  /// 栈转换
...
// morestack -> newstack -> scheduler -> newm -> gc
TEXT runtime·morestack(SB),NOSPLIT,$0-0
	...
	NOP	SP	// tell vet SP changed - stop checking offsets /// NOP无操作数bai，所以称为空操作。执行duNOP指令只使程序计数器PC加1，所以占用一zhi个机器周期
...
TEXT runtime·goexit(SB),NOSPLIT,$0-0
	BYTE	$0x90	// NOP
	CALL	runtime·goexit1(SB)	// does not return  /// funcPC(goexit) + sys.PCQuantum

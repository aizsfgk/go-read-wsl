...
/*
	1. 给这个协程设置一个的魔数(stackguard)。
       每个函数的入口会比较当前栈寄存器值和stackguard值来决定是否触发morestack函数。（这是一个抢占调度点）
	2. 协程调用函数的时候，会检查是否需要栈扩容。如果被设置了抢占标示，那么就会首先调用到
	3. 调用newstack，在newstack里面判断是否是特殊值，这种特殊值，目的不在于扩容，而在于让出调度
 */
// Called from runtime·morestack when more stack is needed.                    /// runtime·morestack在这被调用
// Allocate larger stack and relocate to new stack.                            /// 分配大栈，迁移到新栈
// Stack growth is multiplicative, for constant amortized cost.
//
// g->atomicstatus will be Grunning or Gscanrunning upon entry.                /// 一旦进入
...
func newstack() {
	/// 获取当前g
	thisg := getg() /// newstack 在g0 上操作
	//println("stack.go=>thisg: ", thisg)
	...
	gp := thisg.m.curg /// 之前运行的g
	//println("stack.go=>gp: ", gp)
	...
	casgstatus(gp, _Grunning, _Gcopystack) /// newstack

	// The concurrent GC will not scan the stack while we are doing the copy since
	// the gp is in a Gcopystack status.
	copystack(gp, newsize) /// 进行栈复制
	if stackDebug >= 1 {
		print("stack grow done\n")
	}
	casgstatus(gp, _Gcopystack, _Grunning) /// newstack
	gogo(&gp.sched)
}
...
/// gobuf : &newg.sched
/// fv : funcval{runtime.main}
func gostartcallfn(gobuf *gobuf, fv *funcval) {
	var fn unsafe.Pointer
	if fv != nil {
		fn = unsafe.Pointer(fv.fn) /// 初始化的时候是 main
	} else {
